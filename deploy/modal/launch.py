#!/usr/bin/env python
"""
Deploy a LoopWeave server to Modal FROM A CONFIG FILE — no Python edits.
...
LoopWeave ignores the `modal:` section (it's stripped before the server sees the config), so
the same YAML still works with `loopweave launch --config` locally / in Docker.
...
DEFAULTS = {
    ...
    "proxy_auth": False,
    "hf_secret": "",
    "hf_volume": "loopweave-hf-cache",
    "ckpt_volume": "loopweave-checkpoints",
    "image": "ghcr.io/anonymous/loopweave:latest",
    "registry_secret": "",
}
...
REMOTE_CONFIG = "/etc/loopweave/loopweave_config.yaml"
VENV_PY = "/root/.loopweave/venv/bin/python"
...
class LoopWeaveServer:
    ...
    def warm(self):
        ...
        # The user's loopweave_config.yaml, embedded at deploy time and written here.
    ...
    def serve(self):
        ...
        subprocess.Popen(
            [VENV_PY, "-m", "loopweave", "launch", "--host", "0.0.0.0", "--port", str(PORT),
...
def _info():
    print(f"LoopWeave app '{APP_NAME}': gpu={GPU} proxy_auth={PROXY_AUTH} "
          f"min/max containers={MIN_CONTAINERS}/{MAX_CONTAINERS}")
"""


def load_config(path: Path) -> dict:
    """Parse + validate the LoopWeave config. Returns the parsed dict (incl. any `modal:` section)."""
    try:
        import yaml
    except Exception:
...
def render_app(infra: dict, config_b64: str) -> str:
    header = "\n".join(
        [
            ...
            "# Edit your loopweave_config.yaml and re-run launch.py instead.",
            "import base64, os, subprocess",
            "import modal",
            "",
            f"APP_NAME = {infra['name']!r}",
            f"LOOPWEAVE_IMAGE = {infra['image']!r}",
            f"GPU = {infra['gpu']!r}",
...
def main() -> None:
    ...
    ap.add_argument("--config", help="Path to a loopweave_config.yaml (required unless --down)")
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="keep at 1 — LoopWeave state lives in one container",
    )
