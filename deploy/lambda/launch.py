#!/usr/bin/env python
"""
Run LoopWeave on a Lambda Cloud GPU instance FROM A CONFIG FILE — mirrors deploy/modal/launch.py.

You edit a standard loopweave_config.yaml (the same file `loopweave launch --config` uses) and run
this script. It validates the config, embeds it, launches a Lambda GPU instance that
self-bootstraps LoopWeave (Docker), and prints how to connect. To change models/keys/options,
...
Security: port 10610 is the LoopWeave API, protected by your `tml-` X-API-Key. Prefer reaching it
...
LOOPWEAVE_IMAGE = "ghcr.io/anonymous/loopweave:latest"
LOOPWEAVE_PORT = 10610
HEALTHZ_PATH = "/api/v1/healthz"

# Container paths (host <dir> bind-mounted at /data; matches the README docker-run convention).
CONTAINER_DATA_DIR = "/data"
CONTAINER_CONFIG_PATH = "/data/loopweave_config.yaml"
...
class LambdaClient:
    ...
    def request(
        ...
        headers = {
            ...
            "User-Agent": "loopweave-launch/1.0",
            **self._auth_header(),
        }
        if body is not None:
...
def config_text(path: Path, data: dict) -> str:
    """The loopweave_config.yaml the server should see, with deploy-infra sections stripped."""
...
def _docker_run(host_data_dir: str, hf_token: str, shm_size: str, image: str) -> str:
    ...
    return (
        f"docker run -d --name loopweave --restart unless-stopped --gpus all --shm-size={shm_size} "
        f"-p {LOOPWEAVE_PORT}:{LOOPWEAVE_PORT} {hf}-e HF_HOME={CONTAINER_HF_CACHE} "
        f"-e LOOPWEAVE_CHECKPOINT_DIR={CONTAINER_CHECKPOINT_DIR} "
        f"-v {host_data_dir}:{CONTAINER_DATA_DIR} "
        f"{image} loopweave launch --host 0.0.0.0 --port {LOOPWEAVE_PORT} "
...
def build_user_data(
    ...
    return (
        "#cloud-config\n"
        "# LoopWeave bootstrap generated by launch.py\n"
        "runcmd:\n"
        f"  - mkdir -p {host_data_dir} {host_data_dir}/hf-cache {host_data_dir}/checkpoints\n"
        f"  - echo {cfg_b64} | base64 -d > {host_data_dir}/loopweave_config.yaml\n"
...
def ssh_bootstrap(
    ...
    remote = (
        ...
        f"echo {cfg_b64} | base64 -d | sudo tee {host_data_dir}/loopweave_config.yaml >/dev/null && "
        f"(which docker || (curl -fsSL https://get.docker.com | sudo sh)) && "
        f"sudo docker rm -f loopweave 2>/dev/null; sudo docker pull {image} && "
...
def _host_data_dir(filesystem: str) -> str:
    ...
    return f"/home/ubuntu/{filesystem}/loopweave-data" if filesystem else "/home/ubuntu/loopweave-data"
...
def _banner(ip: str, api_key: str, model: str, instance_id: str, name: str) -> None:
    print("\n" + "=" * 70)
    print(f"✅ LoopWeave launching on Lambda  (instance {instance_id}, name '{name}', {ip})")
    print("=" * 70)
    ...
    print(f"    ssh -N -L {LOOPWEAVE_PORT}:localhost:{LOOPWEAVE_PORT} ubuntu@{ip}")
    print(f"    # then health-gate:  curl http://localhost:{LOOPWEAVE_PORT}{HEALTHZ_PATH}")
    print("\nThen drive training from your laptop:")
    print("    python examples/personality_sft/train.py \\")
    print(f"        --base-url http://localhost:{LOOPWEAVE_PORT} --api-key {api_key} --model {model}")
...
def do_launch(
    ...
    if args.instance_id:
        ...
        print(f"[reuse] bootstrapping LoopWeave on existing instance {args.instance_id} ({ip}) over SSH")
        if args.dry_run:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--config", help="Path to a loopweave_config.yaml (required to launch)")
    ...
    ap.add_argument("--name", default=None, help="instance name (default: loopweave-server)")
    ap.add_argument(
