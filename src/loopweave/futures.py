...
_get_tracer = lambda: get_tracer("loopweave.futures")  # noqa: E731
...
class FutureRecord(BaseModel):
    ...
    error: LoopWeaveException | None = None
        ...
        *,
        ...
        async def _runner() -> None:
            ...
            with _get_tracer().start_as_current_span("future_store.execute_operation") as span:
                span.set_attribute("loopweave.request_id", record.request_id)
                span.set_attribute("loopweave.operation_type", operation_type or "unknown")
                if model_id:
                    span.set_attribute("loopweave.model_id", model_id)
                ...
                try:
                    ...
                    logger.error(
                        "Future failed (LoopWeaveException): %s op=%s",
                        record.request_id,
                        operation_type,
                        exc_info=True,
                    )
