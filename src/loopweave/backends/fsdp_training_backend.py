...
from loopweave.loss_fn import get_loss_fn
...
# =============================================================================
# Data and loss adaptation: Datum -> TensorDict, loopweave loss -> engine loss_function
# =============================================================================
...
def _make_verl_loss_fn(
    ...
    """Wrap loopweave get_loss_fn to engine signature (model_output, data) -> (loss, metrics)."""
    loss_fn_config = loss_fn_config or {}
    loopweave_loss = get_loss_fn(loss_fn_name)
    ...
    def _loss_function(
        ...
        **kwargs: Any,
        ...
        loss, metrics = loopweave_loss(loss_fn_inputs, loss_fn_config)
        return loss, metrics
...
class FSDPTrainingBackend(BaseTrainingBackend):
    ...
    async def async_init(self) -> None:
        ...
        use_ray = os.environ.get("LOOPWEAVE_FSDP_NO_RAY") != "1"
        if not use_ray and n_gpus != 1:
            raise ValueError(
                "LOOPWEAVE_FSDP_NO_RAY=1 (no Ray) requires fsdp_num_gpus=1. "
        ...
        if not _venv or not _venv.strip():
            ...
            # Ray uses py_executable so worker uses venv Python; else node Ray may not find loopweave
        ...
        if self._worker is not None:
            ...
            if len(data) < n_actors:
                raise ValueError(
                    ...
                    f"size or upstream chunking, or set fsdp_num_gpus=1 in loopweave_config.yaml."
                )
