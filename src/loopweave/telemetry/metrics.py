"""Metrics utilities for LoopWeave.
...
def get_meter(name: str = "loopweave"):
...
class LoopWeaveMetrics:
    ...
    def __init__(self):
        ...
        self.redis_operation_duration = meter.create_histogram(
            "loopweave.redis.operation.duration",
            description="Redis operation duration",
            unit="s",
        )

    @classmethod
    def get_instance(cls) -> "LoopWeaveMetrics":
...
def get_metrics() -> LoopWeaveMetrics:
    """Get the LoopWeave metrics instance."""
    return LoopWeaveMetrics.get_instance()
...
class ResourceMetricsCollector:
    ...
    def _setup_metrics(self) -> None:
        ...
        meter = metrics.get_meter("loopweave.resources")
        ...
        if self._gpu_available:
            meter.create_observable_gauge(
                "loopweave.resource.gpu.utilization_percent",
                callbacks=[self._gpu_utilization_callback],
                description="GPU utilization percentage",
                unit="%",
            )
            meter.create_observable_gauge(
                "loopweave.resource.gpu.memory_used_bytes",
                callbacks=[self._gpu_memory_used_callback],
                description="GPU memory used in bytes",
                unit="bytes",
            )
            meter.create_observable_gauge(
                "loopweave.resource.gpu.memory_total_bytes",
