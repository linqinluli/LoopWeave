...
from loopweave.config import TelemetryConfig
...
def _is_debug_mode() -> bool:
    """Check if debug mode is enabled via LOOPWEAVE_OTEL_DEBUG environment variable."""
    return os.getenv("LOOPWEAVE_OTEL_DEBUG", "0") == "1"
...
def _get_otlp_endpoint(config: TelemetryConfig) -> str | None:
    ...
    return os.getenv("LOOPWEAVE_OTLP_ENDPOINT")
...
def init_telemetry(config: TelemetryConfig) -> None:
    ...
    Args:
        config: Telemetry configuration from loopweave.config.TelemetryConfig.
    """
