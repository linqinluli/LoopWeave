...
This guide demonstrates **reinforcement learning (RL)** fine-tuning on the **Countdown** dataset using a **running LoopWeave server**. Full runnable code is in the `examples/countdown_rl/countdown_rl.ipynb` notebook. Although this is a general RL guide, it also documents common issues users may encounter when using LoopWeave for RL and provides step-by-step guidance to help them successfully complete an end-to-end run.
...
3. How to run a minimal **GRPO-like** RL loop in LoopWeave (group sampling + normalized advantages + importance sampling loss)
...
Unlike the Tinker, LoopWeave can run on local GPUs; the experiments below were conducted on a local 2× NVIDIA A100-SXM4-80GB setup (Driver 550.54.15, CUDA 12.9). Before running the example, follow the [Installation Guide](../getting-started/installation.md) to start the LoopWeave server locally.

Key LoopWeave calls (full code in `examples/countdown_rl/countdown_rl.ipynb`):
```python
import tinker
