...
LoopWeave supports **optional persistence** for server state. When enabled, LoopWeave can recover key runtime metadata (sessions, training runs, sampling sessions, futures) after a server restart, and then **reconstruct model runtime state from checkpoints on disk**.
...
uv pip install --override /tmp/loopweave-verl-git-override.txt "loopweave[persistence]>=0.1.8"
```

### Enable persistence

Add a `persistence` section to your `loopweave_config.yaml`:
...
persistence:
  mode: FILE
  file_path: "~/.cache/loopweave/file_redis.json"
  namespace: "persistence-loopweave-server"
```
...
Persistence is configured via the `persistence` section in your `loopweave_config.yaml` configuration file. The following options are available:
...
| `file_path` | string | `~/.cache/loopweave/file_redis.json` | JSON file path (only used when `mode: FILE`) |
| `namespace` | string | `persistence-loopweave-server` | Key namespace prefix for Redis keys |
...
persistence:
  mode: REDIS
  redis_url: "redis://localhost:6379/0"
  namespace: "my-loopweave-deployment"
...
LoopWeave exposes three modes via `persistence.mode`:
...
LoopWeave persists **metadata for major server subsystems** incrementally as changes occur. Specifically, the following subsystems have their state persisted:
...
```bash
loopweave clear persistence --config /path/to/loopweave_config.yaml
```
...
- **Cause**: you restarted LoopWeave with a config whose signature differs from the stored signature in the same namespace.
- **Fix**:
...
- LoopWeave **does not** re-run pending tasks after a crash. Pending work is treated as unsafe and must be retried.
...
All keys are prefixed by a configurable `namespace` (default: `persistence-loopweave-server`) and use `::` as the separator:
...
With the default namespace, LoopWeave uses these major key families:

- **Sessions**  
  `persistence-loopweave-server::session::{session_id}`

- **Training runs**  
  `persistence-loopweave-server::training_run::{training_run_id}`

- **Training checkpoints metadata** (nested under training runs)  
  `persistence-loopweave-server::training_run::{training_run_id}::ckpt::{checkpoint_id}`

- **Sampler checkpoints metadata** (nested under training runs)  
  `persistence-loopweave-server::training_run::{training_run_id}::sampler_ckpt::{checkpoint_id}`

- **Sampling sessions**  
  `persistence-loopweave-server::sampling_session::{sampling_session_id}`

- **Futures** (TTL-based)  
  `persistence-loopweave-server::future::{request_id}`

- **Config signature**  
  `persistence-loopweave-server::config_signature`
...
When persistence is enabled, LoopWeave validates the current `AppConfig` against the stored configuration signature **before** launching the server. This is designed to prevent a restart from silently interpreting old state with a new incompatible config.

If a mismatch is detected, LoopWeave aborts startup with a fatal error and shows a diff.
...
At the end of Phase 1, LoopWeave has restored *metadata*, but model runtime state (adapters/weights in GPU memory) is not yet reconstructed.

### Phase 2: checkpoint-based recovery (async init)

After controller restore, LoopWeave performs checkpoint-based recovery:
...
This means LoopWeave guarantees:
...
> Important: sequence IDs remain **monotonically increasing** even across restarts. LoopWeave does not "rewind" `next_seq_id` to the checkpoint boundary, by design.

---

## Safety checks

LoopWeave includes a restart-safety check to prevent silent corruption when configuration changes across restarts.

### Config signature validation (restart safety)

LoopWeave stores a `ConfigSignature` derived from `AppConfig.get_config_for_persistence()` (notably excluding the persistence config itself).

On startup, LoopWeave compares selected fields (default: `SUPPORTED_MODELS`) and can be configured to check additional fields via `check_fields`:
