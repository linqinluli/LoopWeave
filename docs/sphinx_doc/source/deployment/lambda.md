...
takes you end to end — configure a LoopWeave server, launch it on a Lambda GPU, train a "talk
...
A LoopWeave server running on a Lambda GPU VM (auto-provisioned and self-bootstrapped via Docker),
...
2. **The LoopWeave repo.** The deploy helper talks to Lambda's HTTP API; it only needs `pyyaml`
...
The deploy helper [`deploy/lambda/launch.py`](https://github.com/anonymous/LoopWeave/tree/main/deploy/lambda/launch.py)
is **config-file driven** and mirrors the Modal launcher: you edit a standard
`loopweave_config.yaml` and run the script. Lambda infra goes in an optional `lambda:` section
that is stripped before the server sees it.

Save this as `yoda_lambda.yaml`:

```yaml
checkpoint_dir: ~/.cache/loopweave/checkpoints   # mapped to the VM's /data; see Step 4 about durability
...
# Lambda Cloud infra for deploy/lambda/launch.py (LoopWeave ignores this; it's stripped before the server sees it):
lambda:
  gpu: a100              # family hint; auto-pick prefers a100
  name: loopweave-yoda
  # ssh_key: my-key      # default: your account's sole registered key
  # filesystem: loopweave     # Lambda persistent filesystem for durable checkpoints (else ephemeral root disk)
```
...
The cheapest Lambda GPU (`gpu_1x_a10`, sm_86) has a known issue in the current LoopWeave image:
...
matches your `gpu:` hint, provisions it, and self-bootstraps LoopWeave in Docker via cloud-init
...
```text
[launch] gpu_1x_a100_sxm4 in us-east-1 (~$1.99/hr), ssh_key=my-key, name=loopweave-yoda
...
server's `checkpoint_dir`, which on the VM lives under `/home/ubuntu/loopweave-data/checkpoints`.
The script prints a `run_id` — copy the adapter off the instance with `scp`:

```bash
scp -r ubuntu@203.0.113.45:/home/ubuntu/loopweave-data/checkpoints/<run_id> ./weights/
...
# or by name:
python deploy/lambda/launch.py --down --name loopweave-yoda
```
...
time**. Because LoopWeave is multi-tenant, you can point **several concurrent jobs or users** at the
