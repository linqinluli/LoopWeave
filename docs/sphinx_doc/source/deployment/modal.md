...
takes you end to end — configure a LoopWeave server, deploy it to Modal, train a "talk like
...
A LoopWeave server running on a Modal L4 GPU, reachable at a public `…modal.run` URL, that you
fine-tune from your laptop using the runnable code in
[`examples/personality_sft/`](https://github.com/anonymous/LoopWeave/tree/main/examples/personality_sft).
```
...
2. **The LoopWeave repo.** The deploy helper and the training example live in the repo; the
...
The deploy helper [`deploy/modal/launch.py`](https://github.com/anonymous/LoopWeave/tree/main/deploy/modal/launch.py)
is **config-file driven**: you edit a standard `loopweave_config.yaml` (the same file
`loopweave launch --config` uses) and run the script. Modal infra goes in an optional `modal:`
section that is stripped before the server sees it.

Save this as `yoda_modal.yaml`:

```yaml
checkpoint_dir: ~/.cache/loopweave/checkpoints   # on Modal, launch.py pins this to a Volume automatically
...
# Modal infra for deploy/modal/launch.py (LoopWeave ignores this; it's stripped before the server sees it):
modal:
  gpu: L4
  name: loopweave-yoda
...
[`deploy/modal/loopweave_config.example.yaml`](https://github.com/anonymous/LoopWeave/tree/main/deploy/modal/loopweave_config.example.yaml),
and the example used below ships its own
[`config.yaml`](https://github.com/anonymous/LoopWeave/tree/main/examples/personality_sft/config.yaml).
```
...
```text
✓ Created web endpoint => https://<your-workspace>--loopweave-yoda-loopweaveserver-serve.modal.run
```
...
```bash
curl https://<your-workspace>--loopweave-yoda-loopweaveserver-serve.modal.run/api/v1/healthz
# {"status":"ok"}
```
...
python deploy/modal/launch.py --down --name loopweave-yoda           # stop it later
```

`checkpoint_dir` is automatically pinned to a [Modal Volume](https://modal.com/docs/guide/volumes)
(`loopweave-checkpoints`) so your adapters survive container shutdown.

## Step 3 — Train the Yoda LoRA from your laptop

The training script [`examples/personality_sft/train.py`](https://github.com/anonymous/LoopWeave/tree/main/examples/personality_sft/train.py)
...
[`dataset.py`](https://github.com/anonymous/LoopWeave/tree/main/examples/personality_sft/dataset.py).
...
python examples/personality_sft/train.py \
    --base-url https://<your-workspace>--loopweave-yoda-loopweaveserver-serve.modal.run \
...
server's `checkpoint_dir`, which on Modal is the `loopweave-checkpoints`
...
```bash
modal volume get loopweave-checkpoints <run_id> ./weights/
...
```bash
python deploy/modal/launch.py --down --name loopweave-yoda
```

With `min_containers: 0`, an idle Modal deployment already costs nothing, but `--down`
removes the app entirely. Your adapters remain in the `loopweave-checkpoints` Volume until you
delete it.

```{tip}
Even with scale-to-zero, the GPU is **under-utilized** during an active session — the client
waits between bursts. Because LoopWeave is multi-tenant, you can keep one warm deployment busy by
