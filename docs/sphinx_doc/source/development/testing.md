...
LoopWeave supports running tests on both CPU and GPU devices. The test suite includes unit tests, integration tests, and persistence tests. Below are the instructions for writing and running tests.
...
The `conftest.py` automatically sets the environment variable `LOOPWEAVE_CPU_TEST=1` when `--gpu` is not specified, which configures backends to use CPU-compatible implementations during testing. Tests marked with `@pytest.mark.gpu` are automatically skipped.
...
1. Set a model path via the `LOOPWEAVE_TEST_MODEL` environment variable
2. Start a Ray cluster
3. Run pytest with the `--gpu` option

```bash
export LOOPWEAVE_TEST_MODEL=/path/to/your/model
ray start --head
...
LoopWeave tests support persistence via Redis or FileRedis. By default, persistence is **enabled** for all tests:
...
docker run -d --name loopweave-test-redis -p 6379:6379 redis:7-alpine
...
- `set_cpu_env`: (autouse) Sets `LOOPWEAVE_CPU_TEST=1` when not running in GPU mode
...
export LOOPWEAVE_TEST_MODEL=/path/to/model
ray start --head
uv run pytest tests -v -s --gpu
