...
you like, then point this at it:
    ...
    # local:
    loopweave launch --host 0.0.0.0 --port 10610 --config your_config.yaml
    ...
    python examples/personality_sft/train.py \
        --base-url https://<workspace>--loopweave-server-loopweaveserver-serve.modal.run \
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Personality (Yoda) LoRA SFT against a LoopWeave server")
    ...
    print(f"       modal volume get loopweave-checkpoints {run_id} ./weights/")
