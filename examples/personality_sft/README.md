...
running LoopWeave server (it does not start one). The same `train.py` works whether the server
is local or on Modal.

```
dataset.py   synthetic "talk like Yoda" pairs + assistant-only-masked Datum builder
train.py     connects to a LoopWeave server, samples before/after, trains the LoRA, saves it
```
...
**Local:**
```bash
loopweave launch --host 0.0.0.0 --port 10610 --config examples/personality_sft/config.yaml
```
...
# prints a URL like https://<workspace>--personality-sft-loopweaveserver-serve.modal.run
```
...
# auto-picks a GPU, bootstraps LoopWeave, and prints an SSH-tunnel command + connect details.
