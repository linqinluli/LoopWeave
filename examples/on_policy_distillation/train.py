"""
On-policy distillation (OPD) against a running LoopWeave server.

This is a TRAINING-ONLY client script: it connects to a RUNNING LoopWeave server (it does NOT
...
you like, then point this at it:
    ...
    python examples/on_policy_distillation/train.py \
        --base-url https://<workspace>--on-policy-distillation-loopweaveserver-serve.modal.run \
        --api-key tml-...
...
def main() -> None:
    ap = argparse.ArgumentParser(description="On-policy distillation against a LoopWeave server")
    ...
    print(f"    modal volume get loopweave-checkpoints {run_id} ./weights/", flush=True)
