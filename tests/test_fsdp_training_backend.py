...
Integration tests (GPU, optional LOOPWEAVE_TEST_MODEL):
...
def test_config_to_worker_dict():
    ...
    from loopweave.backends.fsdp_training_backend import _config_to_worker_dict
...
def test_slot_pool_config_get_lora_alpha():
    ...
    from loopweave.backends.fsdp_training_backend import SlotPoolConfig
...
async def test_async_init_raises_when_no_ray_and_multi_gpu():
    """async_init raises ValueError when LOOPWEAVE_FSDP_NO_RAY=1 and fsdp_num_gpus != 1."""
    from loopweave.backends.fsdp_training_backend import FSDPTrainingBackend

    prev = os.environ.get("LOOPWEAVE_FSDP_NO_RAY")
    os.environ["LOOPWEAVE_FSDP_NO_RAY"] = "1"
    try:
        config = ModelConfig(
            model_name="test",
            model_path=Path("/tmp/model"),
        ...
        with pytest.raises(ValueError, match="LOOPWEAVE_FSDP_NO_RAY=1.*fsdp_num_gpus=1"):
            await backend.async_init()
    finally:
        if prev is None:
            os.environ.pop("LOOPWEAVE_FSDP_NO_RAY", None)
        else:
            os.environ["LOOPWEAVE_FSDP_NO_RAY"] = prev


def test_fsdp_port_allocation_by_index():
    """Ports 29500, 29501, ... by FSDP model order; backends get correct fsdp_index."""
    from loopweave.backends.base_backend import BaseTrainingBackend

    # Need real backend path so _fsdp_index is set; restore env after
    saved = os.environ.pop("LOOPWEAVE_CPU_TEST", None)
    try:
        model_configs = [
            ModelConfig(
                model_name="model_a",
        ...
        if saved is not None:
            os.environ["LOOPWEAVE_CPU_TEST"] = saved
...
async def test_fsdp_colocate_actor_uses_training_gpu_fraction(monkeypatch):
    ...
    monkeypatch.delenv("LOOPWEAVE_FSDP_NO_RAY", raising=False)
...
def test_chunk_tensordict_allow_2d_nested():
    ...
    from loopweave.backends.fsdp_training_backend import _chunk_tensordict_allow_2d_nested
...
def test_datum_list_to_tensordict_keys_and_shapes():
    ...
    from loopweave.backends.fsdp_training_backend import _datum_list_to_tensordict
...
def test_make_verl_loss_fn_signature():
    ...
    from loopweave.backends.fsdp_training_backend import _make_verl_loss_fn
...
# -----------------------------------------------------------------------------
# Integration tests: FSDP backend single-process (GPU, LOOPWEAVE_TEST_MODEL)
# -----------------------------------------------------------------------------
...
async def test_fsdp_backend_single_process_create_forward_optim_save_load():
    """
    FSDPTrainingBackend with LOOPWEAVE_FSDP_NO_RAY=1 and 1 GPU: async_init, create_adapter,
    forward, optim_step, save_state, load_state. Requires LOOPWEAVE_TEST_MODEL and CUDA.
    """
    if "LOOPWEAVE_TEST_MODEL" not in os.environ:
        pytest.skip("Set LOOPWEAVE_TEST_MODEL for FSDP integration test")

    prev_no_ray = os.environ.get("LOOPWEAVE_FSDP_NO_RAY")
    os.environ["LOOPWEAVE_FSDP_NO_RAY"] = "1"
    try:
        ...
        if prev_no_ray is None:
            ...
            os.environ["LOOPWEAVE_FSDP_NO_RAY"] = prev_no_ray
...
async def _run_fsdp_single_process_flow() -> None:
    ...
    model_path = Path(os.environ["LOOPWEAVE_TEST_MODEL"])
    config = ModelConfig(
...
def test_shard_list_preserves_order_even_split():
    ...
    from loopweave.backends.fsdp_training_backend import _shard_list
