...
from loopweave.exceptions import ConfigMismatchError, UnknownModelException
from loopweave.futures import FutureStore
from loopweave.persistence import (
    ConfigCheckField,
    flush_all_data,
    get_redis_store,
...
from loopweave.persistence.redis_store import (
    DEFAULT_CHECK_FIELDS,
    ConfigSignature,
    has_existing_data,
    load_config_signature,
)
from loopweave.sampling_controller import SamplingController, SamplingSessionRecord
from loopweave.state import ServerState, SessionManager
from loopweave.training_controller import TrainingController, TrainingRunRecord


def _is_gpu_mode() -> bool:
    """Check if running in GPU mode (not CPU test mode)."""
    return os.environ.get("LOOPWEAVE_CPU_TEST") != "1"


def _create_test_config(checkpoint_dir: Path) -> AppConfig:
    """Create a minimal test config.

    In GPU mode, uses the real model path from LOOPWEAVE_TEST_MODEL environment variable.
    In CPU mode, uses a dummy model path since the model won't actually be loaded.
    """
    if _is_gpu_mode():
        assert "LOOPWEAVE_TEST_MODEL" in os.environ, (
            "Environment variable LOOPWEAVE_TEST_MODEL must be set for GPU tests."
        )
...
class TestSamplingSessionPersistence:
    ...
    def test_sampling_session_with_history_persisted(self, setup):
        ...
        from loopweave.sampling_controller import SamplingHistoryEntry
...
def _create_config_with_models(
    ...
    from loopweave.persistence import PersistenceConfig, PersistenceMode
    from loopweave.persistence.redis_store import DEFAULT_CHECK_FIELDS
