...
def _get_file_redis_path(test_name: str) -> Path:
    ...
    if _test_temp_dir is None:
        _test_temp_dir = Path(tempfile.mkdtemp(prefix="loopweave_test_"))
...
def _clear_redis_db(redis_url: str) -> None:
    ...
    try:
        ...
        warnings.warn(
            f"[loopweave tests] Failed to clear Redis database: {e}",
            RuntimeWarning,
            stacklevel=2,
        )
...
def set_cpu_env(request):
    if not request.config.getoption("--gpu"):
        os.environ["LOOPWEAVE_CPU_TEST"] = "1"
...
def configure_persistence(request):
    ...
    """
    from loopweave.persistence import PersistenceConfig, get_redis_store
    ...
    if no_persistence:
        store.configure(PersistenceConfig(namespace="loopweave_test"))  # mode=disabled by default
    else:
        ...
        if _redis_available() and TEST_REDIS_URL is not None:
            ...
            store.configure(PersistenceConfig.from_redis_url(TEST_REDIS_URL, namespace="loopweave_test"))
        else:
            # Redis not available - fall back to FileRedis with unique temp file per test
            test_name = request.node.name
            file_path = _get_file_redis_path(test_name)
            warnings.warn(
                f"[loopweave tests] Redis unavailable; falling back to FileRedis at {file_path}",
                RuntimeWarning,
                stacklevel=2,
            )
            store.configure(
