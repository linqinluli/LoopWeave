...
from loopweave.auth import User
from loopweave.config import AppConfig, ModelConfig, TelemetryConfig
from loopweave.state import ServerState
from loopweave.telemetry.tracing import clear_tracers
...
def session_tracer_provider():
    ...
    tracer_provider = TracerProvider(resource=Resource.create({"service.name": "loopweave-test"}))
...
def _build_state(tmp_path, use_gpu: bool = False) -> ServerState:
    model_path = (
        Path(os.environ.get("LOOPWEAVE_TEST_MODEL", "/path/to/model"))
        if use_gpu
    ...
    config.telemetry = TelemetryConfig(enabled=True, service_name="loopweave-test")
...
async def test_training_workflow_spans(
    ...
    assert len(_get_spans_with_attr(spans, "loopweave.training_run_id", rid)) >= 4
    assert len(_get_spans_with_attr(spans, "loopweave.session_id", session_id)) >= 4

    # Verify create_model span attributes
    create_spans = [s for s in spans if s.name == "training_controller.create_model"]
    assert create_spans[0].attributes["loopweave.training_run_id"] == rid
    assert create_spans[0].attributes["loopweave.base_model"] == "Qwen/Qwen3-0.6B"
...
async def test_sampling_workflow_spans(
    ...
    assert len(_get_spans_with_attr(spans, "loopweave.sampling_session_id", sampling_id)) >= 2
    assert len(_get_spans_with_attr(spans, "loopweave.session_id", session_id)) >= 2
...
async def test_multi_session_and_training_isolation(
    ...
    spans = span_exporter.get_finished_spans()
    s1_spans = _get_spans_with_attr(spans, "loopweave.session_id", sid1)
    s2_spans = _get_spans_with_attr(spans, "loopweave.session_id", sid2)
    ...
    for s in s1_spans:
        if "loopweave.training_run_id" in s.attributes:
            assert s.attributes["loopweave.training_run_id"] == t1.training_run_id
    for s in s2_spans:
        if "loopweave.training_run_id" in s.attributes:
            assert s.attributes["loopweave.training_run_id"] == t2.training_run_id
...
async def test_multi_training_run_same_session(
    ...
    spans = span_exporter.get_finished_spans()
    t1_spans = _get_spans_with_attr(spans, "loopweave.training_run_id", t1.training_run_id)
    t2_spans = _get_spans_with_attr(spans, "loopweave.training_run_id", t2.training_run_id)
    ...
    for s in t1_spans + t2_spans:
        if "loopweave.session_id" in s.attributes:
            assert s.attributes["loopweave.session_id"] == sid
...
async def test_multi_sampling_session_isolation(
    ...
    spans = span_exporter.get_finished_spans()
    ss1_spans = _get_spans_with_attr(spans, "loopweave.sampling_session_id", ss1)
    ss2_spans = _get_spans_with_attr(spans, "loopweave.sampling_session_id", ss2)
...
async def test_end_to_end_with_future_store(
    ...
    # Verify attributes
    assert len(_get_spans_with_attr(spans, "loopweave.session_id", sid)) >= 5
    assert len(_get_spans_with_attr(spans, "loopweave.training_run_id", rid)) >= 4
    assert len(_get_spans_with_attr(spans, "loopweave.sampling_session_id", ss_id)) >= 2

    # Verify FutureStore span attributes
    fs_spans = [s for s in spans if s.name == "future_store.execute_operation"]
    assert fs_spans[0].attributes.get("loopweave.request_id") is not None
    assert fs_spans[0].attributes.get("loopweave.operation_type") == "forward_backward"
...
def telemetry_server(tmp_path_factory, request, span_exporter, setup_tracer_provider):  # type: ignore[misc]
    ...
    if request.config.getoption("--gpu"):
        ray.init(ignore_reinit_error=True)
        model_path = Path(os.environ.get("LOOPWEAVE_TEST_MODEL", "Qwen/Qwen3-0.6B"))
    else:
    ...
    config.telemetry = TelemetryConfig(enabled=True, service_name="loopweave-integration-test")
...
def test_http_trace_isolation(telemetry_server: str, span_exporter, setup_tracer_provider) -> None:
    ...
    try:
        ...
        run_ids = {
            s.attributes["loopweave.training_run_id"]
            for s in spans
            if s.attributes and "loopweave.training_run_id" in s.attributes
        }
        if len(run_ids) >= 2:
            for rid in run_ids:
                assert len(_get_spans_with_attr(spans, "loopweave.training_run_id", rid)) >= 1
    finally:
