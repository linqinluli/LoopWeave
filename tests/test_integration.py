...
How to run this test (GPU required):
    ...
    LOOPWEAVE_TEST_MODEL=/path/to/model/Qwen3-0.6B \\
...
Notes:
    ...
    - In CI without GPUs or without LOOPWEAVE_TEST_MODEL, the test will skip and not fail.
"""


def _get_model_path(env_var: str = "LOOPWEAVE_TEST_MODEL") -> Path:
...
def hf_server_endpoint(tmp_path_factory: pytest.TempPathFactory):
    ...
    # Support both LOOPWEAVE_TEST_MODEL_1/2 and fallback to LOOPWEAVE_TEST_MODEL
    model_envs = ["LOOPWEAVE_TEST_MODEL_1", "LOOPWEAVE_TEST_MODEL_2"]
    models = []
    for env in model_envs:
        if env not in os.environ and "LOOPWEAVE_TEST_MODEL" not in os.environ:
            warnings.warn(
        ...
        models.append(Path(os.environ.get(env, os.environ.get("LOOPWEAVE_TEST_MODEL", ""))))
...
def test_auth_and_pig_latin_training_flow(hf_server_endpoint: str) -> None:
    ...
    tokenizer = AutoTokenizer.from_pretrained(os.environ["LOOPWEAVE_TEST_MODEL"])
    try:
