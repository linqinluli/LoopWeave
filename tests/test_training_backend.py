...
from loopweave.backends.training_backend import HFTrainingBackend
...
def _construct_data(name: str = "extended") -> List[types.Datum]:
    assert "LOOPWEAVE_TEST_MODEL" in os.environ, (
        "Environment variable LOOPWEAVE_TEST_MODEL must be set for this test."
    )
...
async def test_training_backend():
    ...
    model_path = Path(os.environ.get("LOOPWEAVE_TEST_MODEL", "Qwen/Qwen3-8B"))
...
async def test_colocate_sampling_and_training():
    from loopweave.backends.sampling_backend import VLLMSamplingBackend
    from loopweave.backends.training_backend import HFTrainingBackend
