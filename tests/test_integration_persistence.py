...
from loopweave.persistence import PersistenceConfig
...
def test_checkpoint_resume_persistence(tmp_path: Path) -> None:
    ...
    model_env = os.environ.get("LOOPWEAVE_TEST_MODEL")
    if not model_env:
        warnings.warn(
            "Skipping GPU integration test because LOOPWEAVE_TEST_MODEL is not set.",
            RuntimeWarning,
            stacklevel=2,
        )
        pytest.skip("LOOPWEAVE_TEST_MODEL is not set, skipping GPU integration test")
